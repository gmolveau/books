...
C"est une bonne chose que les stories soient petites, elles sont ainsi finies plus vite. Mais cela a fait apparaître deux problèmes :
...
Cet agenda suppose que l'équipe soit formée à Scrum avant le prélude.
...
> S'il s'avère qu'une partie prenante ou un coéquipier a des réticences à signer, qu'est ce qu'on fait ?
>
...
- Développement dirigé par les hypothèses
...
L'équipe possède un backlog initial, ordonné. Les stories les plus prioritaires vont être affinées afin d'être prêtes pour le premier sprint.
...
Spike : dans le cas où les développeurs font remonter qu'il y a plusieurs solutions techniques pour réaliser la story, on la décompose en deux parties : une première pour étudier les solutions et en choisir une, la seconde pour réaliser avec la solution choisie.
...
Après examen des stories d'investissement (eg. technique), l'équipe peut estimer qu'un résumé de cette partie sera utile lors de la présentation aux parties prenantes.
...
La démonstration d'une story peut être faite par un développeur ou confiée au Product Owner; il est bien placé pour savoir comment s'adresser aux parties prenantes.
...
La variante de revue en trois parties :
...
Le bilan de projet, c'est l'occasion de revenir sur la façon dont il s'est déroulé et de dresser une liste des faits marquants. L'idée est d'apprendre pour en faire profiter d'autres.

Malheureusement, le bilan de projet arrive toujours trop tard : le prof est fini, l'équipe déjà dispersée. C'est dommage pour les participants de ne pas avoir appris plus tôt des leçons tirées de cette expérience.
...
- Attributs de contexte (questions à se poser)
  ...
  - Souhaitons-nous pérenniser et renforcer l'agilité ?
  - L'équipe est co-localisée ?
...
L'apport essentiel des pratiques de développement logiciel dans ce domaine vient d'eXtreme Programming (XP).

#### 19.2.1. Intégration continue

Si l'intégration continue ne passe pas, l'équipe s'arrête pour identifier rapidement le problème à l'origine de l'erreur et relancer le build: il ne faut pas laisser une intégration continue « cassée», pour éviter de propager des erreurs. (cf. Hypothèse de la vitre brisée).
...
La pratique du développement piloté consiste à écrire les tests avant d'écrire le code et de profiter de la présence de tests automatiques pour améliorer le code en toute sécurité.
...
Un défaut trouvé sur une story en cours de développement dans le sprint est une condition d'acceptation en échec : la story n'est pas finie. Les membres de l'équipe qui s'occupent de la story ajoute simplement la tâche necessaire pour le corriger.

Cela ne va pas dans le backlog et encore moins dans un outil de bugtracking. Ce n'est donc pas considéré comme un bug.
...
![scrum inversion triangle agile](./files/SCRUM_inversion_triangle_classique_agile.jpg)

#### 20.2.2. Vélocité et capacité

Définition : La vélocité est la mesure de la partie du backlog qui a été réalisée par l'équipe pendant un sprint. La capacité est la prévision de ce que l'équipe devrait être en mesure de faire pendant un sprint.

#### 20.2.3. Backlog et planification

#### 20.2.4. Quand planifier ?

Il n'y a pas de réunion dédiée à la planification de saison.
...
- Pour commencer la séance, il faut définir l'étalon. C'est simpliement une story connue de tous, pour laquelle l'équipe décide en commun d'une valeur arbitraire. Il est préférable de choisir une story de taille moyenne pour laisser ainsi le spectre ouvert vers le haut et vers le bas.
...
Parfois cela n'a pas grand sens d'avoir un choix et une échelle aussi important que la suite de Fibonacci tronquée à 13. Une technique plus simple est d'utiliser 3 tailles de t-shirt : S, M et L.
...
Ce mouvement veut faire prendre conscience que les estimations dans le développement logiciel demandent pas mal d'effort et apportent finalement peu de valeur.
...
Pour les urgences, un objectif raisonnable est d'arriver à une seule à la fois.
...
- l'ajout de stories pendant le sprint nécessite une bonne maîtrise de la définition de prêt;
...
1. Le **temps de traversée** est la durée que met un élément (une fonctionnalité) à parcourir la chaîne complète, à partir de la formulation de la demande jusqu'à son utilisation en service.
2. Le **temps de cycle** est la durée que met un élément à parcourir le workflow ou une partie de celui-ci, qui sera celle sur lqauelle une équipe pourra s'engager une fois que les mesures analysées montreront une stabilité.

La mesure à faire est celle de la durée entre le moment où un élément devient prêt et celui où il est fini.

### 21.5. Arrêter Scrum pour Kanban ?

Kanban, ce n'est pas juste des colonnes avc des Post-it dedans.
...
Arrête de commencer, commencer à finir.
...
Scrum à grande échelle doit rester un outil convivial, en évitant de créer de nouveaux rôles qui ont forcément tendance à prendre du pouvoir aux coéquipiers.
...
S'il fallait choisir un framework agile, ce serait **LeSS**. Car ce que propose LeSS, c'est exactement cela, du Scrum + quelques patterns pour ce contexte de grande taille.
...
De la même façon que les sprints sont synchronisés, les saisons des équipes le sont aussi: même date de début, même date de fin, tout le monde suit le même rythme.

En fin de saison, deux points sont à noter: - une synchronisation poussée a lieu entre les équipes, qui passent du temps ensemble, pour la rétrospective et la planification;
- la constitution des équipes évolue selon le besoin.
...
On parle de guildes et de squads. On essaie de copier sans tenir compte du contexte. A tel point que certains ont voulu en faire un modèle pour l'agilité à grande échelle.

Il ne s'agit pas d'un modèle à suivre mais simplement d'une expérience très intéressante, dans un contexte spécifique.
...
![agile fluency model claude aubry](./files/agile_fluency_model.jpeg)
